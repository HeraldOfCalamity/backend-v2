...
from app.domain.entities.paciente_entity import PacienteAutoCreate, PacienteCreate, PacienteCreateWithUser, PacienteOut, PacienteUpdate, PacienteUpdateWithUser
...
@router.post('/admin', response_model=PacienteOut, dependencies=[
    Depends(require_permission('create_patients')),
    Depends(require_permission('create_users')),
])
async def admin_crea_paciente(payload: PacienteCreateWithUser, ctx=Depends(get_user_and_tenant)):
    user, tenant_id = ctx
...
@router.put('/admin/{paciente_id}', response_model=PacienteOut, dependencies=[
    Depends(require_permission('update_users')),
    Depends(require_permission('update_patients')),
])
async def admin_edita_paciente(paciente_id: str, payload: PacienteUpdateWithUser, ctx=Depends(get_user_and_tenant)):
    user, tenant_id = ctx
